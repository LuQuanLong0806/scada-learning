...
PAGES = [
    ...
    dict(slug="terms", kind="support", file="术语翻译表_前端→工业.md",
         title="📖 术语翻译表 · 前端→工业", sub="读文档卡词就查这张表",
         what="60+ 工业黑话→前端等价物（回环≈msw、回滞≈debounce、粘包≈WebSocket 攒 buffer…）+ 仅 8 个真新概念清单", src="📖 黑话词典"),
    ...
    dict(slug="coach-plan", kind="support", file="1v1教练_项目吃透计划.md",
         title="🎓 1对1教练 · 项目吃透计划", sub="7 课沿数据流吃透 DaqMonitor",
         what="L1-L7 每课：读什么文件/断点动手/关卡题/验收，含 MC 运控系列 P0-P4 优先级与调库路线打法", src="🎓 教练导读"),
    dict(slug="intranet", kind="support", file="内网学习完全手册.md",
         title="📴 内网学习完全手册", sub="无外网环境的完整学习/练习方案",
         what="三档环境+NuGet 离线包全家福(17 包对照表)+本地包源配置+每日 SOP+零妥协练习方案+回家同步仪式", src="📴 内网专用"),
    ...
    dict(slug="make-or-use", kind="support", file="手搓vs调库_防过度学习清单.md",
         title="⚖️ 手搓 vs 调库 · 防过度学习清单", sub="哪些调库/哪些必写/哪些只讲不写/哪些不学",
         what="企业 16 类现成库清单 + 10 项必写能力 + 6 个嘴上功夫概念 + 10 条明确不学警告，对齐教练计划三档", src="⚖️ 防过度学习"),
