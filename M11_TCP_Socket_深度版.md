...
public class TcpDevice : DeviceBase
{
    ...
    private CancellationTokenSource? _cts;  // 控制 RecvLoop 取消
    ...
    public override void Connect()
    {
        ...
        _ = RecvLoop(_cts.Token);   // 接口同步返回,收包循环在后台跑(与 SimulatedDevice 同套路)
    }
    public override void Disconnect()
    {
        _cts?.Cancel();
        _heartbeat?.Stop(); _ns?.Close(); _client?.Close();
    ...
    private async Task RecvLoop(CancellationToken ct)
    {
        ...
        while (_ns is { } ns && !ct.IsCancellationRequested)
        {
            ...
            foreach (var body in FrameParser.TryParseFrames(_acc))
                RaiseData(Id, body[0]);   // RaiseData(pointId, value),Timestamp 由它统一打;简化:首字节当值,真实按协议拆
        }
        // 退出循环 = 掉线, 触发重连 (TcpHeartbeatHost 也会判活触发, 双保险)
        if (!ct.IsCancellationRequested) _ = ReconnectAsync(ct);
    }
    private async Task ReconnectAsync(CancellationToken ct)
    {
        State = DeviceState.Offline;
        // 简化版重连: 真实场景用 M9 的 Retry.ExecuteAsync 指数退避(参考工程 TcpDevice 用 1/2/4/8/16s)
