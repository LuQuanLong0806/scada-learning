...
public class TcpDevice : DeviceBase
{
    ...
    private CancellationTokenSource? _recvCts;   // 控制 RecvLoop 取消
    ...
    {
        _client = new TcpClient(); await _client.ConnectAsync(_host, _port, ct);
    ...
    private async Task RecvLoop(CancellationToken ct)
    {
        ...
        while (_ns is { } ns && !ct.IsCancellationRequested)
        {
            ...
            foreach (var body in FrameParser.TryParseFrames(_acc))
                RaiseData(new SensorPoint(Id, body[0]));   // ⚠️ 走构造函数,简化: 首字节当值,真实按协议拆
        }
    ...
    private async Task ReconnectAsync(CancellationToken ct)
    {
        ...
        try { await ConnectAsync(ct); } catch { /* 重连失败, 等 Retry 再试 */ }
    }
}
```
