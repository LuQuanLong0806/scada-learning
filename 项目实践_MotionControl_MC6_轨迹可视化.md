...
#### 🏗️ 为什么这样设计:轨迹图为什么自绘(Panel + OnPaint + GDI),而不是用 Chart 控件或 LiveCharts?

**当时面临的选择**:

| 方案 | 优点 | 代价 |
|---|---|---|
| Chart/LiveCharts 画 XY 散点连线 | 现成,轴/图例白送 | 坐标系是"数据轴",不是"机床坐标系"(Y 要向上、原点在左下);几千点连线重绘,库动画开销大;机床十字线/当前点高亮都要和库搏斗 |
| 自绘 Panel:存点 + OnPaint 画(选定) | 百余行 | 轴刻度、缩放这些"轮子"自己造(本项目只要十字定位线,够用) |

**为什么选它**:轨迹图要的不是"图表",是**俯视图的机床坐标系**——X 向右、Y 向上、毫米等比例(圆要画成圆,不能被拉伸),还有十字线、当前点、越界边框变色这些领域专属画法。自绘把映射公式拿在手里:`屏x = ox + px * scale`、`屏y = oy - py * scale`(那个**减号**就是 Y 轴翻转),机床语义和像素语义一一对应,任何效果改 OnPaint 即得。性能也是自绘主场:4000 点 `DrawLines` 一次成帧 + DoubleBuffered 整帧贴屏,Chart 库的动画/命中测试全是白背的开销。前端类比:要一个"数据可视化"用 ECharts;要一个"游戏小地图",你一定会自己拿 canvas 画——这里是后者。

**不这样会怎样**:Chart 版先要说服库接受"Y 轴反向 + 等比坐标",再逐个关掉图例/动画/悬停提示;两周后客户说"当前点画个大红点、超程边框变红"——每加一个效果都在和库的抽象搏斗。

**🎤 面试一句话**:"轨迹图我自绘不用 Chart 库:要的是机床俯视坐标系——Y 向上、毫米等比、十字定位线,这些是领域语义不是图表语义,自绘把坐标映射拿在手里一个减号就能翻转 Y 轴;4000 点 DrawLines 一帧加双缓冲,比图表库少背动画和交互的开销。"
