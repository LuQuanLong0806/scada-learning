...
| 场景 | 怎么用这份文档 |
|---|---|
| **A. 学 M0 前扫一遍**(推荐) | 每个陷阱读「一句话讲清楚」+「拟人秒懂」就行,**不用细抠代码**,知道有这些坑存在 → 进 M0 时遇到再回来查 |
| **B. M0 中途卡住了** | M0 Day X 卡住,翻到「📌 温故知新」找对应陷阱号 → 精读那一节 + 三档练习 |

**这份文档的代码块不需要你照着敲**—— 它们是**演示陷阱**的,**不是项目代码**。你要敲的项目代码全在 M0-M14 各模块里。这份文档是"防坑地图",读一遍知道哪里有雷,真正踩到雷时回来对照着排雷。
