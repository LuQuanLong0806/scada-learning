...
### 🏗️ 为什么这样设计:协议解析为什么独立成一层,而不是写进设备类里?

**当时面临的选择**:

| 方案 | 优点 | 代价 |
|---|---|---|
| 解析逻辑写在设备类里(收到字节就地拆) | 少建几个文件 | 解析要 IO 才能测;换传输(串口↔TCP)时解析跟着重写;设备类越来越胖 |
| 独立 Protocol 层、纯函数(选定) | 多两个文件夹 | 字节要在层间传递(多一步显式转换) |

**为什么选它**:R2 的 IDevice 拆的是"**谁**提供数据",R3 拆的是"**字节怎么变成数值**"——两个变化方向。解析算法是纯数学(给定字节必得给定结果),把它从 IO 里剥出来,**不需要串口/网络就能穷举测试**:半包、粘包、坏 CRC、异常码……本篇 13 个测试没有一个碰硬件。换设备时协议层原样复用,换协议时设备壳不动。

**不这样会怎样**:解析混在设备类里,测试要先 mock 串口再喂字节;现场抓到一包可疑数据,想单独调试拆包逻辑,得把整个设备拖起来。

**🎤 面试一句话**:"协议解析我独立成纯函数层:字节进、数值出,零 IO 依赖——所以半包粘包、坏帧、字节序这些坑全部可以用纯单测覆盖,不用插硬件;换传输方式解析层原样复用。"
...
#### 🏗️ 为什么这样设计:CRC 为什么自己写 15 行,而不是用 NuGet 库?为什么是 CRC 不是累加校验和?

**当时面临的选择**:

| 方案 | 优点 | 代价 |
|---|---|---|
| 累加校验和(所有字节相加取低 8 位) | 3 行,最快 | `01 02` 和 `02 01`、`00 03` 和 `03 00` 和全部加起来相同的组合全撞车——**换序/对称错误检测不到** |
| NuGet 现成 CRC 库 | 不用手写 | 引依赖只为 15 行代码;库的参数变体(初值/多项式/异或输出)还要对文档 |
| 手写移位版 CRC16-Modbus(选定) | 多写 15 行 | 必须用已知向量验证正确性 |

**为什么选它**:Modbus 的 CRC16 是**规范定死的**(多项式 0xA001、初值 0xFFFF、低字节在前)——自己写才有参数可控,和文档逐条对上;手写版正好是"逐位异或移位"教科书实现,配已知向量测试(① 的第 1 步就是拿规范样例帧验指纹)保证正确。工业现场干扰是**连发多位翻转**,累加和抓不住换序错误,CRC 的指纹特性是任何 1 位翻转必然改变结果——这是 Modbus 40 年选它的原因。

**不这样会怎样**:用累加和,现场静电干扰把两个字节换了位置,校验照样通过——上位机拿到错数据还以为设备坏了,排查方向全错。

**🎤 面试一句话**:"帧校验用 CRC16 不是累加和:累加和检测不到字节换序,CRC 的指纹特性保证任意 1 位翻转必被抓住。实现是规范定死的 0xA001 移位异或,15 行手写加已知向量测试,比引一个库再对参数更可控。"
...
#### 🏗️ 为什么这样设计:拆帧为什么是"流式蓄水池"(Feed + TryTakeFrame),而不是一次性的 Parse(byte[])?

**当时面临的选择**:

| 方案 | 优点 | 代价 |
|---|---|---|
| `Parse(byte[] frame)` 一次到位 | API 直白 | 假设"收到的恰好是完整一帧"——串口/TCP **根本不保证**,半包粘包下直接废 |
| 流式:Feed 攒缓冲 + TryTakeFrame 循环切(选定) | 能处理任意切割:半帧、粘包、帧头前有垃圾 | 类要有状态(缓冲跨调用记忆),多两个方法 |

**为什么选它**:底层传输的语义是**字节流**,不是"消息流"——串口一次 Read 可能只有半帧,TCP 一次可能收到三帧粘着。流式设计把"进货"和"取货"分开:Feed 只管倒进蓄水池,TryTakeFrame 只管尽力切,切不出就停手等下批。**这是所有串口/网络协议解析的标准形态**,Modbus 库、Redis 协议、HTTP chunked 内部都是这个套路。前端类比:WebSocket onmessage 里必须自己攒 buffer 切完整 JSON消息——一模一样的问题。

**不这样会怎样**:一次性 Parse 在真实串口上,半包时返回失败、粘包时丢掉后面几帧——测试环境(整帧喂入)全绿,现场随机丢数据,是最难排查的那类 bug。

**🎤 面试一句话**:"拆帧我做成流式:传输层给的是字节流不保证帧边界,所以 Feed 先攒缓冲、TryTakeFrame 循环切,半包就等下批、粘包就多切几条。一次性 Parse 只在测试里成立,现场必丢数据。"
...
#### 🏗️ 为什么这样设计:串口帧靠"帧头 AA55 + CRC"找边界,TCP 帧为什么改用"长度前缀"?状态为什么从类里挪给调用方?

**当时面临的选择(帧边界)**:

| 方案 | 优点 | 代价 |
|---|---|---|
| 帧头 AA55 定界 | 串口没有长度也可同步 | 载荷里出现 AA 55 就可能"假同步",要靠 CRC 兜底反复试错 |
| 2 字节长度前缀(选定,TCP 场景) | 读头部即知帧长,一次定位 | 双方必须严格按同一格式组帧 |

**为什么选它**:串口是"裸字节+干扰"环境,只能用魔数帧头硬同步;TCP 是**可靠有序**的字节流(没有位翻转、不乱序),长度前缀能一次定位帧边界,载荷内容随便出现什么字节都不怕。**传输介质决定帧格式**——这不是口味问题,是物理层的约束。至于无状态化(缓冲上交给调用方):TCP 客户端往往自己就有接收缓冲管理,解析器做成纯函数 `TryParse(缓冲, out 帧)`,同一个类可以被多个连接复用,测试也不用先 new、再 Feed、再取——一步到位。

**不这样会怎样**:TCP 上仍用帧头同步,载荷含 AA 55 时反复假同步+CRC 试错,白白浪费;解析器自带缓冲则每个连接要 new 一个实例,还多一层"缓冲里剩了什么"的隐藏状态。

**🎤 面试一句话**:"帧格式跟传输介质走:串口不可靠,用 AA55 帧头加 CRC 硬同步;TCP 可靠有序,用长度前缀一次定位。解析器本身做成无状态纯函数,缓冲由调用方维护——一个类服务多个连接,测试也简单。"
