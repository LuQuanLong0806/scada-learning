...
📚 **知识点**
- **接口是合同,加条款要全体签约方补签**:往 `IMotionCard` 加一个方法,所有实现类必须补实现 —— 否则编译不过(CS0535)。这是接口演进的代价,也是它的保护:谁都没法"悄悄不实现"。前端类比:给公共 `interface Props` 加必填字段,所有用到它的组件立刻类型报错,被迫一起改 —— 疼一下,但绝不漏。
- **平行数组签名 `axes[] / targets[]`**:两个数组按下标一一对应(axes[0] 配 targets[0])—— 真卡 SDK 常用这种风格(句柄数组 + 参数数组)。另一种做法是传 `AxisTarget[]` 对象数组,类型更安全但多一个类型声明 —— 演示"简单直接"和"类型安全"的取舍,两种都常见。
- **`speed` 的语义注释要写清**:插补速度 = "走得最远那根轴"的速度(下面实现里 maxDist 决定)—— 接口注释先把语义钉住,调用方才不会误解成"每根轴各自的速度"。

#### 🏗️ 为什么这样设计:插补为什么做成卡的一个指令(MoveLinear),而不是界面端定时循环、每次给两轴各发一小步 MoveAbsolute?

**当时面临的选择**:

| 方案 | 优点 | 代价 |
|---|---|---|
| UI 软件插补:定时器每拍算好两轴下一个小目标,各发一次单轴指令 | 不用动卡 | 两轴指令**先后发出**,每拍都有微小错拍;定时器被 UI 卡顿一毫秒,轨迹就歪;通信/线程开销 ×步数;真卡时代整套作废 |
| 插补下推为卡的指令(选定) | 接口加一个方法、所有实现类补签 | 卡内实现要管等比推进(数学) |

**为什么选它**:插补的本质要求是**两轴严格同步**——"同时启动、等比推进、同时到位"。这个同步放在谁手里,谁就得有时钟权威:放在 UI 端,消息泵一卡轨迹就变形;放在卡内,一个循环里两轴同拍推进(第 2 步实现:同一 i、锁内连续写两个位置),同步由结构保证。这个形状还**对齐行业现实**:真卡(固高/雷赛)的插补就是卡上硬件做的(G01 直线插补),上位机只发一条"从这到那、多快";把 MoveLinear 放进 IMotionCard,将来换真卡 = 换一个实现,窗体一行不改。**"UI 只表达意图,同步交给有权威的一方"**——前端同款道理:动画同步交给 CSS/合成器线程,不用 JS setInterval 逐帧改两个元素。

**不这样会怎样**:软件插补版在画轨迹图(MC6)时,两轴位置事件交错到达,画出来的"直线"全是锯齿——同步缺口肉眼可见。

**🎤 面试一句话**:"插补我做成卡指令而不是界面循环发单轴小步:插补要的是两轴严格同步,UI 消息泵一卡顿轨迹就歪;卡内一个循环同拍推进两轴,同步由结构保证。这也对齐真卡——插补在卡上硬件做,上位机只发一条指令,换真卡窗体不改。"
...
📚 **知识点(界面三处各讲一件事)**
- **4a —— btnLinear 不进控件数组**:控件数组按"每轴一个"组织(btnEnable[i]、btnJog[i]…),而插补按钮**驱动两根轴**,没有"自己的轴" —— 单独订阅、单独写方法。判断标准:进不进数组,看控件是否随轴增长。
- **4b —— 四拍子模板零成本复用**:`SpeedOf → MoveLinear → Fail → AppendLog`,和 MC3 的单轴方法一个节奏 —— 接口层的统一让 UI 层的新指令也长得一样,这就是"模式"的价值:会读一个就会读全部。
- **4c —— 新按钮的可用性加在唯一真源里**:RefreshUiState 是 MC3 立的规矩,新按钮的规则**只能**加在这里,不写进 click handler —— v1 式"各改各的"漏禁 bug 在结构上继续不可能。条件是两个单轴 operable 的"与":插补是绑腿跑,任何一轴不可用,按钮就得灰 —— 界面规则和卡的检查链(第 2 步的逐轴检查)语义对齐。
...
📚 **知识点 —— WinForms"加一个按钮"的完整清单**
- **四处 = 一件事**:① `new` 实例化(5a)、② `Controls.Add` 挂进容器(5b)、③ 属性块 定位置/大小/文案(5c)、④ 字段声明(5d)—— 缺任何一件:漏 ① 编译不过(用了未赋值字段)、漏 ② 不显示、漏 ③ 左上角一个默认小按钮、漏 ④ 编译不过。**WinForms 没有"声明式加按钮",每个控件都是这四件套** —— 对比 React 加按钮只写一处 JSX,就能理解后来 WPF/前端为什么都走向声明式。
- **5c 的坐标是算出来的**:`Location(20, 548)` 正好在 lblSoftLimit1(516)之下、GroupBox 高度(677)之内 —— 手工布局的代价:每个新控件都要自己找空位,放歪了压住别的控件。
- **`TabIndex = 15` 顺延**:轴 1 组最后一个控件是 14,新按钮排 15 —— Tab 键序和视觉顺序保持一致,操作员用键盘也能顺出合理路径(车间里戴手套点不动鼠标,键盘操作真实存在)。
