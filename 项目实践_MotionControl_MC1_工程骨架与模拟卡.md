...
#### 🏗️ 为什么这样设计:运动指令为什么"失败"走返回码,不走异常?

**当时面临的选择**:

| 方案 | 优点 | 代价 |
|---|---|---|
| 抛异常(MotionException) | C# "正统"风格,失败自动中断流程 | **指令失败是业务常态**——没使能、超软限位、断线,现场每小时都在发生;每个调用点 try-catch,正常逻辑淹没在异常处理里 |
| `MotionResult` 枚举返回码(选定) | 多定义一个枚举 | 调用方可能忘了检查返回值(靠代码规约+测试兜住) |

**为什么选它**:异常的正确语义是"**不该发生的事**"(空引用、配置损坏);而"伺服没使能动不了"是**每天都在发生的预期业务结果**。返回码让每个调用点显式写 `if (r != MotionResult.Ok)`,失败处理和正常流程一样是一等公民;枚举值还**对齐真卡 SDK**(固高/雷赛都是 int 返回、0 成功负数失败)——将来换真卡,上层判断逻辑一行不改。这是"错误处理策略跟着错误性质走",不是"语言提供了异常就什么都用异常"。

**不这样会怎样**:异常版本里 UI 每个按钮 handler 都包 try-catch,而且 catch 里还要把异常翻译回"哪根轴为什么失败"给操作员看——等于把返回码的信息用更贵的方式重新发明一遍。

**🎤 面试一句话**:"运动指令我用返回码不用异常:没使能、超限位这些'失败'是业务常态不是异常路径,返回码让失败处理显式平铺;枚举负数值还和固高/雷赛真卡 SDK 对齐,换真卡上层不改。"
...
#### 🏗️ 为什么这样设计:模拟"一段运动"为什么用"节拍循环",而不是 Sleep 到终点一步到位?

**当时面临的选择**:

| 方案 | 优点 | 代价 |
|---|---|---|
| `Thread.Sleep(走完耗时)` 后直接把位置设成终点 | 3 行写完 | 位置"瞬移":界面数字和轨迹图没有中间过程;急停/松手没法中途打断;测试只能验证"最终到了",验证不了过程 |
| 每节拍(如 10ms)推进一步的小循环(选定) | 多写一个循环体 | CPU 每"轴"多一个轻循环(可忽略) |

**为什么选它**:模拟卡要模仿的不是"结果",是**过程**——真卡走一段定位,位置是连续变化的,界面上能看到数字一格格走、轨迹一段段画。节拍循环把"运动"还原成时间轴上的一串中间状态:①UI 每节拍收到 PositionChanged,演示效果和真卡一致;②**急停/点动松手 = CancellationToken.Cancel**,循环当场退出、位置就地冻结——这正是真卡急停的语义;③测试能断言中间状态("走到一半时急停,位置停在半路"),而不只断言终点。**模拟的价值在于像真,而不在于省事**——省事版模拟会掩盖真机才有的时序问题。

**不这样会怎样**:Sleep 版模拟卡全绿,换真卡第一次试急停就发现 UI 逻辑从没处理过"运动中途"的状态——模拟期欠的债,真机期连本带利还。

**🎤 面试一句话**:"模拟卡我用节拍循环不用 Sleep 直达:真卡的运动是连续过程,界面要看中间状态、急停要能中途打断、测试要能断言'停在了半路'——模拟的价值在于像真,省事版模拟会在换真机时把时序问题一次性爆出来。"
