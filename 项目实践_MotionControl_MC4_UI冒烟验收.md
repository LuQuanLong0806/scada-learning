...
#### 🏗️ 为什么这样设计:UI 冒烟为什么要费这么大劲自动化(STA 线程+手动泵+异常回传),而不是发版前人肉点一遍?

**当时面临的选择**:

| 方案 | 优点 | 代价 |
|---|---|---|
| 手动验收清单(人点一遍) | 零开发成本 | 每次改动都要重点;人点 20 步必偷懒漏步;"我这边没问题"不是证据 |
| UI 冒烟自动化:真窗体+真事件流跑全流程(选定) | 约 90 行测试代码 | 要啃 STA/消息泵三个底层概念 |

**为什么选它**:这个测试测的是**接线**,不是逻辑——MC2 已经验证了卡的行为,MC3 手写的窗体代码(控件数组反查、边沿检测、InvokeRequired 切换)恰恰是**没有测试覆盖的部分**,而"接线错误"只有把窗体真跑起来才暴露。自动化的价值不在"跑得快",在**每次提交都跑**:MC3 以后每次改窗体,`dotnet test` 第 13 个用例都会把"连接→使能→点动→定位→急停"全流程真点一遍。90 行换"界面接线永不倒退",这笔账划算。前端类比:Playwright 冒烟一条链路 vs 全手点,同一个决策。

**为什么异常要回传**:测试线程和 UI 线程是两条命,UI 线程抛的异常 xUnit 默认**看不见**(测试照样绿!)——所以必须在 STA 线程里 try-catch 存起来、Join 之后由测试线程断言"整个流程零异常"。不回传的 UI 测试是假安全。

**🎤 面试一句话**:"窗体我做了冒烟自动化:真开 STA 线程跑真窗体,DoEvents 手动驱动消息泵,操作员全流程走完断言零异常——MC2 测的是卡的行为,这个测的是窗体接线,每次提交自动回归。关键是 UI 线程异常必须显式回传测试线程,否则测试线程看不见异常照样假绿。"
